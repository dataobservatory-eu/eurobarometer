...
Because `eurobarometer` is in an early development phase, you cannot install it from CRAN with the `install.packages("eurobarometer")` command yet. For installing a not-yet-released package, you not only need R to be installed on your computer, but must be able to build packages from source (those that need compilation of C/C++ or Fortran code).  
